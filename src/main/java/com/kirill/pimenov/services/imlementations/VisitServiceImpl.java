...
public class VisitServiceImpl implements VisitService {
    ...
    public ResponseEntity<?> save(Long pageId, Long visitorId, LocalDate localDate) {
        ...
        newVisit.setDate(localDate==null?LocalDate.now():localDate);
        newVisit.setPageId(pageId);
    ...
    public ResponseEntity<?> getVisitStatsForPeriod(LocalDate dateFrom, LocalDate dateTo) {
        ...
        Set<Long> visitorsInRange = visitRepository.findVisitorIdByDateBetween(dateFrom, dateTo);
        long regularVisitorCounter = 0;
        for (Long visitorId: visitorsInRange){
            if(visitRepository.countVisitorPages(visitorId, dateFrom, dateTo) >= 10)
